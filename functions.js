...
const controlElectionText = (electionText, rte) => {
  if ((MONTH + 3) % ELECTION_MONTH_INTERVAL === 0) {
    ...
    ((MONTH - 2) % ELECTION_MONTH_INTERVAL === 0 ||
      (MONTH - 3) % ELECTION_MONTH_INTERVAL === 0) &&
    ELECTION_COUNT !== 0
  ) {
    if (ELECTION_COUNT < 6) {
      SPEED += 5;
      rte.scale.set(
        RTE_SCALE + (ELECTION_COUNT * 2) / 5,
        RTE_SCALE + (ELECTION_COUNT * 2) / 5
      );
    }
...
const multipler = (SCORE) => {
  ...
  else if (SCORE > 10) return 9;
  else if (SCORE > 25) return 13;
  else if (SCORE > 50) return 16;
  else if (SCORE > 75) return 21;
};
...
const setup = () => {
  ...
  app.ticker.add((delta) =>
    gameLoop(delta, rte, money, countingText, monthText, electionText)
  );
...
PIXI.loader
  .add(["images/rte.png", "images/money.png"])
...
const updateMonthText = (app, monthText) => {
  let month_text =
    parseInt(MONTH / 12) !== 0
      ? parseInt(MONTH / 12) > 1
        ? `${parseInt(MONTH / 12)} YEARS `
        : `${parseInt(MONTH / 12)} YEAR `
      : "";
